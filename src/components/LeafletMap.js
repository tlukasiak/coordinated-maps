...
var LeafletMap = React.createClass({
	...
	onDragEnd(e) {
		console.log(`[LeafletMap] onDragEnd distance = ${JSON.stringify(e.distance)}`);
		actions.changeLocation(this.map.getCenter());
	},

	onZoomChanged() {
		var zoom = this.map.getZoom();
		console.log(`[LeafletMap] onZoomChanged ${JSON.stringify(zoom)}`);
		actions.changeZoom(zoom);
	},

	componentDidMount() {
		let center = this.props.center;
		console.log(`[LeafletMap] componentDidMount: ${center.lat} ${center.lng}`);
	...
	shouldComponentUpdate(nextProps, nextState) {
		...
		console.log(`[LeafletMap] shouldComponentUpdate nextProps ${JSON.stringify(nextProps)} center ${center.lat}:${center.lng}`);
